namespace GLTech.Processing
{
    public sealed unsafe class FFXAA : Effect, IDisposable
    {
        ...
        {
            tempbuffer = new TextureBuffer(width, height);
            if (threshold > 255)
                sqrThreshold = 255 * 255 * 3;
            else if (threshold < 0)
                sqrThreshold = 0;
            else
                sqrThreshold = threshold * threshold * 3;
        }

        private TextureBuffer tempbuffer;
        ...
        public override void Process(TextureBuffer target)
        {
            ...
            if (!ShowEdges)
            {
                TextureBuffer.BufferCopy(target, tempbuffer);
            ...
            TextureBuffer.BufferCopy(tempbuffer, target);
            return;
