...
namespace GLTech.World.Composed
{
    public partial class BlockMap : Entity
    {
        ...
        public BlockMap(TextureBuffer map, Func<Color, Texture?> mapTexture, TextureFilling textureFilling,
            ...
            void suboptimalMode()
            {
                ...
                for (int column = 0; column < map.Width; column++)
                {
                    for (int line = 0; line < map.Height; line++)
                    {
                        ...
                        if (mapResult is Texture texture)
                            makeBlock(line, column, texture);
                    }
                }
                ...
                void makeBlock(int line, int column, Texture block_texture)
                {
                    ...
                    if (block_texture.Image.Buffer == IntPtr.Zero) return;
                    ...
                    for (int line = 0; line < map.Height; line++)
                    {
                        ...
                        if (nullable is Texture texture && !texture.IsNull)
                        {
                            ...
                            if (isEmpty(column - 1, line))
                                makePlaneOnly(column, line, texture, Side.left);

                            if (isEmpty(column, line + 1))
                                makePlaneOnly(column, line, texture, Side.bottom);

                            if (isEmpty(column + 1, line))
                                makePlaneOnly(column, line, texture, Side.right);

                            if (isEmpty(column, line - 1))
                                makePlaneOnly(column, line, texture, Side.top);
                        }
                    }
                }
            }
